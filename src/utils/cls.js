...
// Existing Pairs
export const encodedPairs = new Set(
  LP_TOKENS.map((p) => encodePair(p.token0_name, p.token1_name))
);
...
export const getTokenPath = (t1, t2) => {
  ...
  if (
    encodedPairs.has(encodePair("BNB", t1)) &&
    encodedPairs.has(encodePair("BNB", t2))
  ) {
    return [t1, "BNB", t2];
  }
  // No path found
...
export const getSwapPath = (token0, token1) => {
  return getTokenPath(token0.title, token1.title).map((item) =>
    tokenMap.get(item)
  );
};
