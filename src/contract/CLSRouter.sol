...
contract CLSRouter is IPancakeRouter02 {
    using SafeMath for uint;
   ...
   receive() external payable {
    ...
    function swapExactTokensForTokens(
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn-fee, path);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path);
        uint fee = amounts[0] * totalETHFee/(10000-totalETHFee);
        require(amounts[0] + fee <= msg.value, 'PancakeRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        if (fee > 0) {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint fee = _feeAmount(amountIn);
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amountIn - fee
        );
