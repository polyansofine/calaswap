  ...
  return (
    ...
    >
      ...
      <DialogTitle>
        <Typography sx={{ mb: 4 }}>
          {type === TYPE_SWAP
            ? `Swap ${token_index === 0 ? "from" : "to"}`
            : `Select the ${token_index === 0 ? "first" : "second"} token`}
        </Typography>
        <Typography sx={{ mb: 4 }}>
          {(token_index === 0 && Object.keys(token0).length === 0) ||
          (token_index === 1 && Object.keys(token1).length === 0)
      ...
      >
        <List>
          {TOKENS.filter((item) => {
            if (type === TYPE_LIQUIDITY) {
              if (
                (token_index === 0 &&
                  ...
                  !encodedPairs.has(encodePair(item.title, token1.title))) ||
                (token_index === 1 &&
                  Object.keys(token1).length === 0 &&
                  !encodedPairs.has(encodePair(item.title, token0.title)))
              ) {
                return false;
              }
            }
            ...
            return (
              !(
                (token_index === 0 && item.title === token1.title) ||
                (token_index === 1 && item.title === token0.title)
              ) &&
              (item.title.toLowerCase().indexOf(searchStr) >= 0 ||
            ...
            <ListItem
              ...
              onClick={() => {
                ...
                if (token_index === 1) {
                  type === TYPE_LIQUIDITY
                    ? dispatch(
                        tokenActions.setTokens(
                          Object.keys(token1).length === 0 ? token0 : {},
                          item
                        )
                      )
                  ...
                  type === TYPE_LIQUIDITY
                    ? dispatch(
                        tokenActions.setTokens(
                          item,
                          Object.keys(token0).length === 0 ? token1 : {}
                        )
                      )
